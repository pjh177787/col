...
class  DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        mfcc_input    = np.zeros((self.batch_size, self.dim[0]+self.kernel_width-1,self.dim[1]+1, self.n_channels))
        delta_input   = np.zeros((self.batch_size, *self.dim))
        zcr_input     = np.zeros((self.batch_size, self.dim[0],1))        
        mfcc_input1   = np.zeros_like(mfcc_input)
        delta_input1  = np.zeros_like(delta_input)
        zcr_input1    = np.zeros_like(zcr_input)        
        mfcc_input2   = np.zeros_like(mfcc_input)
        delta_input2  = np.zeros_like(delta_input)
        zcr_input2    = np.zeros_like(zcr_input)        
        ...
        for i in range(len(list_IDs_temp)): # len(list_IDs_temp) = ceil ( batch_size / num_sample_per_batch )
            ...
            zcr_temp = np.load("./feature_save/"+self.mode+"/"+self.serial+"/"+self.data_list[list_IDs_temp[i]]['mixture_audio_filename']+"_zcr.npy")
            if(zcr_temp.shape[0] > mfcc_temp.shape[0]):
                zcr_temp = zcr_temp[:mfcc_temp.shape[0]]
            zcr = zcr_temp[:,np.newaxis]
            concat = np.hstack((mfcc_temp,zcr))
            mfcc     = concat[:,:,np.newaxis]
            ...
            for j in range(0,num_sample_per_batch):
                ...
                if(self.signature == "_test"):
                
                    mfcc_input1[ i*num_sample_per_batch+j,] = mfcc[j*timestep+1: (j+1)*timestep+self.kernel_width,]
                    #delta_input1[i*num_sample_per_batch+j,] = delta[j*timestep+1: (j+1)*timestep+1,]
                    #zcr_input1[  i*num_sample_per_batch+j,] = zcr[  j*timestep+1: (j+1)*timestep+1,]

                    mfcc_input2[ i*num_sample_per_batch+j,] = mfcc[j*timestep+2: (j+1)*timestep+self.kernel_width+1,]
                    #delta_input2[i*num_sample_per_batch+j,] = delta[j*timestep+2: (j+1)*timestep+2,]
                    #zcr_input2[  i*num_sample_per_batch+j,] = zcr[  j*timestep+2: (j+1)*timestep+2,]
                # Store label
        ...
        if(self.signature == "_train"):
            ...
            X_list = [[mfcc_input],
                      [mfcc_input1],
                      [mfcc_input2]]
        return X_list, y_ret#keras.utils.to_categorical(y, num_classes=2)
